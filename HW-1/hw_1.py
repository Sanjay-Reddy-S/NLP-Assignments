...
def n_word_tokens(word_counts):
    ...
    '''
    total=0
    for type in word_counts.keys():
        total+=word_counts[type]
    return total
...
class NaiveBayes:
    ...
    def update_model(self, bow, label):
        ...
        for token in bow.keys():
            ...
            self.class_total_word_counts[label]+=bow[token] #doubt!!!!
        self.class_total_doc_counts[label]+=1.0
        #self.class_total_word_counts[label]=len(self.class_word_counts[label])
    ...
    def log_likelihood(self, bow, label, alpha):
        ...
        likelihood=0.0
        for token in bow.keys():
            likelihood+=math.log(self.p_word_given_label_and_pseudocount(token,label,alpha))
        return likelihood
            ...
            for f in os.listdir(p):
                with open(os.path.join(p,f),'r') as doc:
                    ...
                    """
                        ...
                        print 'predicted label: '+self.classify(bow,alpha)
                        print content
                        break
                    """
                    total += 1.0
    ...
    def print_wrong_review(self,alpha):
        pos_path = os.path.join(self.test_dir, POS_LABEL)
...
#print tokenize_doc("Buffalo buffalo Buffalo buffalo buffalo buffalo Buffalo buffalo is a correct sentence. Yes !")
